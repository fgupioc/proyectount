...
public class GestionarAreaServicio {
    ...
    public Area obtenerId(List<Area> areas, String nombre) {
        area = new Area();
        int id = 0;
        for (Area tipo : areas) { 
            if (tipo.getDescripcion().equals(nombre)) {
                area.setId(tipo.getId());
                area.setDescripcion(tipo.getDescripcion()); 
            }
        }
        return area;
    }
    public void llenarLista(JTable listado, List<Area> areas) throws Exception {
    ...
    public boolean buscarAreaList(List<Area> categorias, String nombre){
        boolean flag = false; 
        for(Area dt:categorias){           
